...
def test_postfix():
    ...
    # assert myShunt.getPostfix("tanh(sinh(cos(x)))*(csc(sec(x)))^(3sinh(πe))+cot(sin(cosh(5x)))") == ['x', 'cos', 'sinh', 'tanh', 'x', 'sec', 'csc', '3', '3.141592653589793', '2.718281828459045', '*', 'sinh', '*', '5', 'x', '*', 'cosh', 'sin', 'cot', '+', '^', '*'] # fails because this function seems to lose a + sign after shunt yard. Tokens seem to work.ShuntingYard
